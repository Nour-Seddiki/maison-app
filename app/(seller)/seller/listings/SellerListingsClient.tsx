...
export default function SellerListingsClient({ listings }: { listings: ListingRow[] }) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-surface border border-border p-8 max-w-sm w-full mx-4">
            ...
            <div className="flex gap-3 mt-4">
              ...
              <Button variant="ghost" size="sm" onClick={() => { setConfirm(null); setError(null) }}>Cancel</Button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {listings.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-text-muted font-body">
                  No listings yet.{' '}
              ...
              <tr key={listing.id} className="border-b border-border hover:bg-surface-2/50 transition-colors">
                ...
                <td className="px-6 py-4 text-sm text-text-secondary font-body">{listing.inquiryCount}</td>
                <td className="px-6 py-4">
